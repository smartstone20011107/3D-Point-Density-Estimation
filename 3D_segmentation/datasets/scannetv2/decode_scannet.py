...
def create_shm_train(List_name, npy_path):
    for i, list_name in enumerate(List_name):
        ...
        if not os.path.exists("/dev/shm/chi_{}_nl".format(fn)):
            ...
            # ####write share memory
            sa_create("shm://chi_{}_xyz".format(fn), xyz)
            sa_create("shm://chi_{}_rgb".format(fn), rgb)
            sa_create("shm://chi_{}_sem_label".format(fn), sem_label)
            sa_create("shm://chi_{}_ins_label".format(fn), ins_label)
            sa_create("shm://chi_{}_nl".format(fn), nl)
...
def create_shm_val(List_name, npy_path):
    for i, list_name in enumerate(List_name):
        ...
        if not os.path.exists("/dev/shm/chi_{}_nl".format(fn)):
            ...
            sa_create("shm://chi_{}_sem_label".format(fn), sem_label)
            sa_create("shm://chi_{}_ins_label".format(fn), ins_label)
            sa_create("shm://chi_{}_sup".format(fn), sup)
            sa_create("shm://chi_{}_nl".format(fn), nl)
